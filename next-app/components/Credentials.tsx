...
const credentials = [
  {
    ...
    year: "2025",
    description:
      "Understands the foundations of digital computing and programming to understand how to build complex software systems at scale. Also, how modern collaborative software development takes place within organizations and teams through practical application.",
    logo: "/icons/CDLogo.jpeg",
  ...
  },
  {
    ...
    year: "2024",
    description:
      "Understands team accountabilities, events, and artifacts, as well as how to guide your team in the effective application of scrum.",
    logo: "/icons/SA-CSM.png",
...
const CredentialsCarousel = () => {
  ...
  const handleDragEnd = (_: any, info: any) => {
    if (info.offset.x < -100) setCurrent(nextIndex);
    else if (info.offset.x > 100) setCurrent(prevIndex);
  };
  ...
  const renderSideCard = (index: number, position: "left" | "right") => (
    ...
    >
      ...
      <p className="text-lg font-semibold mb-1" style={{ color: "#915f6e" }}>
        {credentials[index].title}
      </p>
  ...
  return (
    <div className="relative w-full h-screen flex flex-col justify-center items-center px-6 py-10">
      ...
      <div className="relative w-full max-w-6xl flex items-center justify-center">
        <button onClick={goToPrevious} className="absolute left-0 z-30 p-3 bg-white rounded-full shadow hover:bg-gray-100">
          <ChevronLeft className="w-6 h-6 text-gray-600" />
        </button>
        <button onClick={goToNext} className="absolute right-0 z-30 p-3 bg-white rounded-full shadow hover:bg-gray-100">
        ...
        <div className="relative flex justify-center items-center w-full h-[560px]">
          ...
          <AnimatePresence initial={false} mode="wait">
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-2" style={{ color: "#915f6e" }}>
                {credentials[current].title}
              </h3>
              <p className="text-md mb-1">{credentials[current].issuer}</p>
              <p className="text-sm mb-4 text-gray-400">{credentials[current].year}</p>
              <p className="text-base leading-relaxed px-4 mb-4 text-gray-300">{credentials[current].description}</p>
              <a href={credentials[current].link} target="_blank" rel="noopener noreferrer" className="mt-auto inline-block text-white text-sm px-4 py-2 rounded transition hover:opacity-80" style={{ backgroundColor: "#915f6e" }}>
                View Certificate
              </a>
            </motion.div>
          </AnimatePresence>
